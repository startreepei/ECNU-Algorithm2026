...
const {
  ...
  normalizeText,
  sanitizeIdentifier
} = require("./src/fretDataset");
const { computeConnectedComponents, computeDependencyMaps } = require("./src/connectedComponents");
const { renderContract } = require("./src/lustre");
const { getDependencyStatus, runSolver } = require("./src/solver");
  ...
  for (const componentName of targetComponents) {
    ...
    for (let index = 0; index < connectedComponents.length; index += 1) {
      ...
      const ccContract = cloneContractWithProperties(
        contract,
        [...cc.properties],
        sanitizeIdentifier(`${componentName}_cc${index}Spec`)
      );
